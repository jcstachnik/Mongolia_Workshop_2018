...
class HypoDDRelocator(object):
    ...
    def _read_event_information(self):
        ...
        for event in catalog:
            ...
            # JCS FIX to be sdobs (travel time resid)
            if origin.quality.standard_error is not None:
                current_event["origin_time_error"] = \
    ...
    def _cross_correlate_picks(self, outfile=None):
        ...
        for event_1, event_2 in event_id_pairs:
            ...
            # JCS calculate distance btwn events, skip if too far..
            #evdist = calc_dist(event_1_dict["origin_latitude"],
            dkm = gps2dist_azimuth(event_1_dict['origin_latitude'],event_1_dict['origin_longitude'],
                                    event_2_dict['origin_latitude'],event_2_dict['origin_longitude'])[0] / 1000.
            msg = "Event_1 {event_id_1} Event_2  {event_id_2} {d} km".format(
                    event_id_1=event_1, event_id_2=event_2, d=dkm )
